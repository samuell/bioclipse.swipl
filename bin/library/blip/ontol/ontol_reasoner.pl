...
show_explanation(obo,_IsRecursive,Fact,RuleName):- % TODO
        !,
        (   Fact <- Body :: RuleName - _Reason),
        writeln(Body),
        nl.                     % TODO
...
subclass(X,Y) <- class_union_element(Y,X) :: isa_from_union/given.
...
disjoint_over(X,Y) <- disjoint_over(Y,X) :: disjoint_over_symmetrical/given.
