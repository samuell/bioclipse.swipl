...
metadata_db:entity_synonym(X,Syn) :-
        rdfid_oboid(U,X),
        anyPropertyAssertion('http://purl.obofoundry.org/obo/IAO_0000111',U,Lit),
        rdf_literal_to_native(Lit,Syn),
        Syn\='',
        \+ entity_label(X,Syn).
metadata_db:entity_synonym(X,Syn) :-
        rdfid_oboid(U,X),
        anyPropertyAssertion('http://purl.obofoundry.org/obo/IAO_0000118',U,Lit),
        rdf_literal_to_native(Lit,Syn),
        Syn\='',
        \+ entity_label(X,Syn).
...
ontol_db:def(X,Def):-
        rdfid_oboid(U,X),
        anyPropertyAssertion('http://purl.obofoundry.org/obo/IAO_0000115',U,Lit),
