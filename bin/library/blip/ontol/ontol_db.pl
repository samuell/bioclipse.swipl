...
%node_link(X,Y,Z):- inst_rel(X,Y,Z).
...
parent_over_nr(R,Via,ID,PID):-
        ...
        \+ (   parent_over(R,ID,IID),
               ...
               ;   subclassT(IID,PID)
               ;   transitive_over(R,InhR), % not in parent_over?? 
                   parent_overT(InhR,IID,PID))).
...
%% relation(?Rel)
% disjunction of property/1 and slot/1
...
%%  slot(?Slot) is nondet.
% declares that Slot is a relation that links instances to datatype
%    equivalent to owl:DatatypeProperty
:- extensional(slot/1).

%%  slot(?Slot,?Name) is nondet.
%  combines and slot/1 entity_label/2
%  holds when slot(?Slot) and entity_label(?Slot,?Name)
:- multifile slot/2.
slot(P,Name):- entity_label(P,Name),slot(P).
...
% @param ?Rel = all_some_all_times ; all_some_some_times ; all_some_tr ; disjoint_over
...
% deprecate?
parent_path(ID,[link(T,IDp)],IDp):-
        tparent(ID,T,IDp).
parent_path(ID,[link(T,IDz)|LL],IDp):-
        tparent(ID,T,IDz),
        parent_path(IDz,LL,IDp).

% DEPRECATED?
% (+,?,?)
% shortest path, compressing chains, removing is_as
parentTmin(ID,P,IDp):-
        setof(P/IDp,parentTcpath(ID,P,IDp),L),
        findall(P1/IDp,
                (
                  member(P/IDp,L),
                  subpathL(P,IDp,L,P1)),
        ...
        member(P/IDp,L),
        not(member(P/IDp,L2)).
parentTcpath(ID,P,IDp):- parentT(ID,TL,IDp),cpath(TL,P).
...
%simple_inst_to_slot_inst_of(C):-
%        inst_of(ID,C),
%        inst_sv(ID,S,RoleID),
%        inst_sv(RoleID,RoleClass),
%        inst_expand(RoleClass).
...
ontology_segment(InNodes,Edges,OutNodes,Opts):-
        ...
        debug(ontol,'Will use ~w for graph traversal',[Pred]),
        % todo: optimize this
...
%% parent_over_oneof(R-Rs,Node,PNode)
% predicates for incremental graph traversal
% (class only: TODO)
