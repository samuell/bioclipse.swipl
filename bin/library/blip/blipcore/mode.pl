...
% eg :- mode foo(+,-) is det
user:term_expansion((:-mode(T is M)),
                    Clauses):-
        (   mode_checking_on
        ->  context_module(Mod),
            T=..[F|ATypes],           % eg [foo,+,-]
