...
% assert is slow; looking up dynpreds is also slow
%  not suitable for large datasets; better with small subsets of search spc

%% memocall(+Goal)
%  call Goal using memoization. semi-deprecated. Consider table_pred/1 instead
memocall(X):-
        X =.. [F|AL],
        not(memo(F)),
        anonvarlist(AL,AL2),
        X2 =.. [F|AL2],
        retractall(memo_cache(F,_)),
        setof(X2,X2,XL),
        setof(X3,(member(X3,XL),assertz(memo_cache(F,X3))),_),
        assertz(memo(F)), % end of transaction
        fail.
memocall(X):-
        X =.. [F|_],
        memo_cache(F,X).
...
read_file_to_tokens(File,Tokens):-
        ...
        tokenize_atom(FileContentsAtom,Tokens).

%% tokenize_atom(+A,?AL)
%  as tokenize_atom/3, uses default_token_list/1
tokenize_atom(A,AL):-
        atom_codes(A,CL),
        default_token_list(TL),
        tokenize_codes(CL,TL,AL).
%% tokenize_atom(+A,+TL,?AL)
%  converts an atom into a list of token atoms
tokenize_atom(A,TL,AL):-
        atom_codes(A,CL),
        tokenize_codes(CL,TL,AL).

%% tokenize_codes(+CL,?AL)
tokenize_codes(CL,AL):-
        default_token_list(TL),
        tokenize_codes(CL,TL,AL).
%% tokenize_codes(+CL,+TL,?AL)
...
tokenize_codes(CL,TL,AL):-
        tokenize_codes(CL,TL,AL,[]).
tokenize_codes([],_,[A],BL):-
        atom_codes_rev(A,BL).
tokenize_codes([C|CL],TL,ALr,BL):-
...
tokenize_codes([C|CL],TL,ALr,BL):-
        ...
        ;   atom_codes_rev(A,BL),
            ALr = [A|AL]),
        tokenize_codes(CL,TL,AL,[C]).
tokenize_codes([C|CL],TL,AL,BL):-
        tokenize_codes(CL,TL,AL,[C|BL]).
