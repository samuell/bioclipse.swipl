...
download_url(InURL,Px):-
        ...
        (   uri_resolution(InURL,URL) % user can set up their own registry
        ->  true
        ;   URL=InURL),
...
age_exceeds_threshold(Age):-
        ...
        max_cached_file_age_seconds(MaxAge),
        Age > MaxAge,
        debug(load,'Age: ~w exceeds threshold: ~w',[Age,MaxAge]),
        !.
age_exceeds_threshold(Age):-
        \+ max_cached_file_age_seconds(_),
        Age > 3600,
        !,
        debug(load,'Age: ~w exceeds default threshold: 3600',[Age]).
...
transform_and_load_xml(Fmt,Mod,File):-
	statistics(cputime, CpuOld),
        ...
        xml_to_preds(Fmt,XML,PL),
        findall(Fact,(member(P,PL),
                      (   P=_:_
                      ->  Fact=P
                      ;   Fact=Mod:P)),
                Facts),
        insert_facts(Facts,[transactional(true)]),
%        forall(member(P,PL),
%               insert_fact(Mod:P)),
%               asserta(Mod:P)),
...
file_to_prolog_cmd(gene_rif,'tbl2p -p gene_rif').
