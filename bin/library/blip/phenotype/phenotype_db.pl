:- module(phenotype_db,
          [
           ...
           feature_pair_best/4,
           feature_pair_simnr/3,
           feature_pair_simnr/6,
...
entity_whole_part_of_ca((E1,W1),(E2,W2),A) :-
        entity_whole_part_of(E1,W1,A),
        entity_whole_part_of(E2,W2,A).
...
%% phenotype_db:feature_pair_simnr(+F1,+F2,?P1Xs:list,?P2Xs:list,?MaxIC,?ICCS)
% =|use_module(phenoblast)|= if you want this to be dynamically calculated.
% otherwise this is assumed extensional / pre-calculated
:- extensional(feature_pair_simnr/6).

%% feature_pair_simnr(F1,F2,Score)
% as feature_pair_simnr/6, omitting phenotype list
feature_pair_simnr(F1,F2,Score) :-
        feature_pair_simnr(F1,F2,_,_,_,Score).

% used?
feature_pair_best(F1,F2,PSub,Freq) :-
        findall(PSub,feature_phenotype_compare(F1,_,F2,_,PSub),PSubsBag),
        setof(PSub,member(PSub,PSubsBag),PSubs),
        setof(Freq-PSub,(member(PSub,PSubs),phenotype_frequency(PSub,Freq)),[Freq-PSub|_]).
...
/** <module> represents phenotypes using EQ model
